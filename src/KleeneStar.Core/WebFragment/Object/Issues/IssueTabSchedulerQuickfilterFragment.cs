...
{
    /// <summary>
    /// Quickfilter header of the issue calendar tab: the personal scopes the calendar can honour
    ...
    /// </summary>
    /// <remarks>
    /// The filters are stored under the issue list's view key — the same one the board and the
    /// list use — so a filter defined here shows on all of them and the other way round. The
    /// selection is written into the shared query state of
    /// <see cref="IssueTabSchedulerFragment"/>, which re-queries the calendar.
    /// </remarks>
    [Section<SectionTabTemplatePrimary>]
    [Scope<IssueTabSchedulerTemplateFragment>]
    [Order(1)]
    [Cache]
    public sealed class IssueTabSchedulerQuickfilterFragment : FragmentControlPanel
    {
        /// <summary>
        /// Represents the unique identifier for the content used in the application.
        /// </summary>
        public static readonly string ContentId = "id_7E4CA163D9056BF8C2E5AB03F61A4975";

        /// <summary>
        /// Gets the quickfilter control offering the view scopes and the user's own filters.
        /// </summary>
        ...
        public IssueTabSchedulerQuickfilterFragment(IFragmentContext fragmentContext)
            : base(fragmentContext)
        {
            ...
            ((IViewStateModelBound)Quickfilter).Resource<IssueSchedulerResource>().Model("filter");
