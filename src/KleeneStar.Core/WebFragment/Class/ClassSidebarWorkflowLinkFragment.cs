...
{
    /// <summary>
    /// Represents a sidebar item link fragment that displays the 'Workflows' link in the class sidebar.
    /// </summary>
    ...
    {
        private static readonly IUri _uri = CoreHub.GetUri<global::KleeneStar.Core.WWW.Workflows._classid_.Index>();
        ...
        /// <summary>
        /// Computes the trailing sidebar badge for the 'workflows' link: the number of
        /// workflows defined on the resolved class. Returns <c>null</c> when no class can be
        /// resolved or the class has no workflows, so an empty collection leaves the link
        /// badge-free rather than showing a noisy zero.
        /// </summary>
        /// <param name="renderContext">
        /// The render context used to resolve the class identifier whose workflows are counted.
        /// Cannot be null.
        /// </param>
        /// <returns>
        ...
        private static string GetBadge(IRenderControlContext renderContext)
        {
            ...
            var count = CoreHub.WorkflowManager.GetWorkflows(new ClassIdParameter(classId)).Count();
