...
{
    /// <summary>
    /// Represents a sidebar item link fragment that displays the 'security levels' link in the class sidebar.
    /// </summary>
    ...
    [Cache]
    public sealed class ClassSidebarSecurityLevelLinkFragment : FragmentControlSidebarItemLink
    {
        private static readonly IUri _uri = CoreHub.GetUri<global::KleeneStar.Core.WWW.SecurityLevels._classid_.Index>();
        ...
        /// Cannot be null.
        /// </param>
        public ClassSidebarSecurityLevelLinkFragment(IFragmentContext fragmentContext)
            : base(fragmentContext)
        {
            Icon = _ => new IconShieldHalved();
            Text = _ => "kleenestar.core:securitylevel.link.label";
        ...
        /// <summary>
        /// Computes the trailing sidebar badge for the 'security levels' link: the number of
        /// levels defined on the resolved class. Returns <c>null</c> when no class can be resolved or
        ...
        private static string GetBadge(IRenderControlContext renderContext)
        {
            ...
            var count = CoreHub.SecurityLevelManager.GetSecurityLevels(new ClassIdParameter(classId)).Count();
