...
namespace KleeneStar.Core.WebFragment.Dashboard
{
    ...
    [Cache]
    public sealed class DashboardViewTableFragment : FragmentControlViewItem
    {
        /// <summary>
        ...
        {
            ServiceFactory = _ => DataServiceDescriptor.TableData(CoreHub.GetUri<global::KleeneStar.Core.WWW.Api._1_.Dashboards.Table>().ToString())
        };
        ...
        public DashboardViewTableFragment(IFragmentContext fragmentContext)
            : base(fragmentContext)
        {
            Icon = _ => new IconTable(TypeIconTheme.Light);
            Title = _ => "kleenestar.core:view.table.title";
            Table.Bind = _ => new Binding()
                .Add(new BindSearch() { Source = DashboardViewSearchFragment.ContentId })
                .Add(new BindFilter())
                .Add(new BindPaging() { Source = DashboardViewPaginationFragment.ContentId });
