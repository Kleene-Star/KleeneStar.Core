...
namespace KleeneStar.Core.WWW.Api._1_.Priorities
{
    ...
    public sealed class State : RestApiSelection<Model.Entities.Priority>
    {
        /// <summary>
        ...
        /// </returns>
        protected override IQueryable<RestApiSelectionItem> RetrieveItems(IQuery<Model.Entities.Priority> query, IQueryContext context, IRequest request)
        {
            var list = new List<RestApiSelectionItem>()
            {
                new()
                {
                    Id = PriorityState.Active.Id(),
                    Text = I18N.Translate(request, PriorityState.Active.Text()),
                    Color = PriorityState.Active.Color()
                },
                new()
                {
                    Id = PriorityState.Archived.Id(),
                    Text = I18N.Translate(request, PriorityState.Archived.Text()),
                    Color = PriorityState.Archived.Color()
                }
            };
