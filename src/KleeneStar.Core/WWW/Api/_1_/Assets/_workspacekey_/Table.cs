...
{
    /// <summary>
    /// Table endpoint of the asset overview: returns the asset-kind objects of the
    ...
    /// paging therefore run in memory over the workspace's assets. The endpoint honours
    ...
    /// and <c>l</c> (page size). The row actions reuse the issue action shells
    /// (<c>/issue/{key}/{edit,clone,delete,favorite}</c>), which operate on the object
    /// by key regardless of its kind.
    /// </remarks>
    [Cache]
    public sealed class Table : IRestApi
    {
        ...
        /// <summary>
        /// Builds the filtered, paged page of the workspace's assets and returns it as
        ...
        public IResponse Get(IRequest request)
        {
            ...
            var assets = GetAssets(workspace?.Id).AsEnumerable();

            // the archived chip flips the lifecycle scope: without it the list shows the
            // active assets, with it the archived history
            ...
            assets = assets.Where(x => x.State == state);

            if (selected.Contains(Quickfilter.StarredId))
            {
                assets = assets.Where(x => starredIds.Contains(x.Id));
            }

            if (selected.Contains(Quickfilter.MineId))
            {
                assets = assets.Where(x => x.AssigneeId == ownerId);
            }

            if (selected.Contains(Quickfilter.CreatedId))
            {
                assets = assets.Where(x => x.CreatorId == ownerId);
            }
        ...
        /// <summary>
        /// Fetches the asset-kind objects of the supplied workspace, most recently
        ...
        /// <returns>The workspace's assets. The list may be empty.</returns>
        private static IReadOnlyList<ObjectEntity> GetAssets(Guid? workspaceId)
        {
            ...
            var query = new Query<ObjectEntity>()
                ...
                .WhereEquals(x => x.Kind, Model.Entities.ObjectKind.Asset);

            return [.. CoreHub.ObjectManager.GetObjects(query)];
        }

        /// <summary>
        /// Builds the (fixed) column definitions of the asset table. The labels are
        ...
        private static IEnumerable<RestApiTableColumn> BuildColumns(IRequest request)
        {
            ...
            {
                Id = "key",
                Label = I18N.Translate(request, "kleenestar.core:object.kind.assets.column.key"),
                Visible = true
            };
            ...
            {
                Id = "summary",
                Label = I18N.Translate(request, "kleenestar.core:object.kind.assets.column.summary"),
                Visible = true
            };
            ...
            {
                Id = "updated",
                Label = I18N.Translate(request, "kleenestar.core:object.kind.assets.column.updated"),
                Visible = true
            };
        }

        /// <summary>
        /// Projects a single asset to a table row, linking the row to the asset detail
        /// page and offering the edit, clone, star-toggle, and delete options.
        /// </summary>
        /// <param name="asset">The asset to project.</param>
        /// <param name="starred">Whether the calling identity has starred the asset.</param>
        /// <param name="request">The request used to resolve localized content and URIs.</param>
        /// <returns>The table row.</returns>
        private static RestApiTableRow BuildRow(ObjectEntity asset, bool starred, IRequest request)
        {
            var uri = CoreHub.GetUri<global::KleeneStar.Core.WWW.Asset._objectkey_.Index>()?
                .BindParameters(new ObjectKeyParameter(asset.Key));
            ...
            return new RestApiTableRow()
            {
                ...
                [
                    ...
                    new RestApiTableCell() { Content = asset.Key },
                    new RestApiTableCell() { Content = asset.Summary },
                    new RestApiTableCell() { Content = asset.Updated.ToString("yyyy-MM-dd HH:mm") }
                ],
                Options = GetOptions(asset, starred, request).Select(o => o.ToJson()),
                Uri = uri?.ToString(),
                Image = asset.Icon?.Uri?.ToString()
            };
        }
        ...
        /// modal. The actions reuse the generic issue action shells, which operate on the
        /// object by key regardless of its kind.
        /// </summary>
        /// <param name="asset">The asset the options act on. Cannot be null.</param>
        /// <param name="starred">Whether the calling identity has starred the asset.</param>
        /// <param name="request">The request used to resolve localized labels and URIs.</param>
        /// <returns>The overflow menu options.</returns>
        private static IEnumerable<RestApiOption> GetOptions(ObjectEntity asset, bool starred, IRequest request)
        {
            var keyParameter = new ObjectKeyParameter(asset.Key);
