...
{
    /// <summary>
    /// Quickfilter endpoint of the asset Kanban board. Offers the personal scopes the board
    /// can honour as toggleable chips — starred assets, assets assigned to the caller and
    /// assets created by the caller — followed by the filters the user defined.
    /// </summary>
    /// <remarks>
    /// Mirrors the issue board's endpoint. The archived scope the asset list offers is
    /// absent, because the board collects active objects only and could not honour it.
    /// <see cref="ObjectKindBoardFilter"/> translates the selected ids back into filters.
    /// </remarks>
    [Cache]
    public sealed class KanbanQuickfilter : RestApiQuickfilter<ObjectEntity>
    {
        /// <summary>
        /// The key under which the quickfilters a user defined for the workspace's assets
        /// are stored.
        /// </summary>
        public const string ViewKey = "assets";
