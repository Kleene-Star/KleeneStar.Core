...
namespace KleeneStar.Core.WWW.Api._1_.Workspaces._key_.Classes._id_
{
    /// <summary>
    /// Provides CRUD operations for class items via a REST API.
    /// </summary>
    [Cache]
    [SegmentGuid<ClassParameter>()]
    public sealed class Index : RestApiCrud<Class>
    {
        ...
        /// </returns>
        protected override IRestApiCrudResultRetrieve RetrieveForClone(IQuery<Class> query, IRequest request)
        {
            ...
            return RetrieveForClone(request, data, "kleenestar.core:class.clone.title");
        }
        ...
        /// </returns>
        protected override IRestApiCrudResultRetrieveDelete RetrieveForDelete(IQuery<Class> query, IRequest request)
        {
            using var context = ModelHub.CreateDbContext();
            var data = CoreHub.ClassManager.GetClasses(query, context)
                .FirstOrDefault();
        ...
        /// </returns>
        protected override IRestApiValidationResult Validate(Class existingItem, RestApiCrudFormData payload, IRequest request)
        {
        ...
        /// </returns>
        protected override IRestApiCrudResultCreate Create(RestApiCrudFormData fieldMap, IRequest request, out Class newItem)
        {
            var id = Guid.NewGuid();
            newItem = new Class(id)
            {
            ...
            CoreHub.ClassManager.AddClass(newItem);
        ...
        /// </returns>
        protected override IRestApiCrudResultCreate Clone(Class existingItem, RestApiCrudFormData fieldMap, IRequest request, out Class newItem)
        {
            var id = Guid.NewGuid();
            newItem = new Class(id)
            {
        ...
        /// The HTTP request providing additional context.
        /// </param>
        protected override IRestApiCrudResultUpdate Update(Class existingItem, RestApiCrudFormData payload, IRequest request)
        {
            var res = base.Update(existingItem, payload, request);

            CoreHub.ClassManager.UpdateClass(existingItem);
        ...
        /// </returns>
        protected override IRestApiCrudResultDelete Delete(Class existingItem, IRequest request)
        {
