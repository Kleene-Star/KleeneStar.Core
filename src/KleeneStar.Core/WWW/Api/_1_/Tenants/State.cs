...
namespace KleeneStar.Core.WWW.Api._1_.Tenants
{
    ...
    /// <remarks>
    [Title("Tenant state")]
    public sealed class State : RestApiSelection<Model.Entities.Tenant>
    {
        /// <summary>
        ...
        /// </returns>
        protected override IQueryable<RestApiSelectionItem> RetrieveItems(IQuery<Model.Entities.Tenant> query, IQueryContext context, IRequest request)
        {
            var list = new List<RestApiSelectionItem>()
            {
                new()
                {
                    Id = TenantState.Active.Id(),
                    Text = I18N.Translate(request, TenantState.Active.Text()),
                    Color = TenantState.Active.Color()
                },
                new()
                {
                    Id = TenantState.Archived.Id(),
                    Text = I18N.Translate(request, TenantState.Archived.Text()),
                    Color = TenantState.Archived.Color()
                }
            };
