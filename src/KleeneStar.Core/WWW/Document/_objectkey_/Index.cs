...
{
    /// <summary>
    /// The reading view of a single document: a clean, publication-style presentation
    /// of the document's title (in the headline) and its rich-text body (contributed by
    /// <see cref="WebFragment.Object.Documents.DocumentReadFragment"/>). The URL is
    /// <c>/document/{objectkey}</c>. Editing happens on the sibling
    /// <see cref="Edit"/> page reached from the headline's edit button.
    /// </summary>
    /// <remarks>
    /// The page is kind-aware: an object whose kind is not <see cref="Model.Entities.ObjectKind.Document"/>
    /// is redirected to the detail view of its own kind, so a link that guessed the wrong
    /// route self-heals instead of rendering a document chrome around, say, an issue.
    /// </remarks>
    ...
    public sealed class Index : IPage<VisualTreeWebApp>, IScopeGeneral
    {
        ...
        public void Process(IRenderContext renderContext, VisualTreeWebApp visualTree)
        {
            ...
            // a link that addressed the wrong route (e.g. /document for an issue) is
            ...
            // record the visit so this document surfaces at the top of the object
            // dropdown's "recently used" list; a detail page is also a subpage of its
            // workspace, so the workspace recency is advanced too
            if (@object is not null)
            {
