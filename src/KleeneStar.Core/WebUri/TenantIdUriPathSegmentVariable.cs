...
namespace KleeneStar.Core.WebUri
{
    ...
    public class TenantIdUriPathSegmentVariable<TParameter> : UriPathSegmentVariableGuid<TParameter>
        where TParameter : IParameterStatic, new()
    {
        ...
        public TenantIdUriPathSegmentVariable(object tag = null)
            : base(tag)
        {
        }
        ...
        {
            return new TenantIdUriPathSegmentVariable<TParameter>(this)
            {
        ...
        public override bool IsMatched(string value)
        {
            ...
            else if (TenantManager.ReservedTenantNames.Contains(value?.Trim().ToLower()))
            {
                return false;
            }
