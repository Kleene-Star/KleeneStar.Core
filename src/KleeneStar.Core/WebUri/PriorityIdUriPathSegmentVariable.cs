...
namespace KleeneStar.Core.WebUri
{
    ...
    public class PriorityIdUriPathSegmentVariable<TParameter> : UriPathSegmentVariableGuid<TParameter>
        where TParameter : IParameterStatic, new()
    {
        ...
        public PriorityIdUriPathSegmentVariable(object tag = null)
            : base(tag)
        {
        }
        ...
        {
            return new PriorityIdUriPathSegmentVariable<TParameter>(this)
            {
        ...
        public override bool IsMatched(string value)
        {
            ...
            else if (PriorityManager.ReservedPriorityNames.Contains(value?.Trim().ToLower()))
            {
                return false;
            }
