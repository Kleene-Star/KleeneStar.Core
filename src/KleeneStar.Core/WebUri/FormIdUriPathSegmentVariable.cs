...
namespace KleeneStar.Core.WebUri
{
    ...
    public class FormIdUriPathSegmentVariable<TParameter> : UriPathSegmentVariableGuid<TParameter>
        where TParameter : IParameterStatic, new()
    {
        ...
        public FormIdUriPathSegmentVariable(object tag = null)
            : base(tag)
        {
        }
        ...
        {
            return new FormIdUriPathSegmentVariable<TParameter>(this)
            {
        ...
        public override bool IsMatched(string value)
        {
            ...
            else if (FormManager.ReservedFormNames.Contains(value?.Trim().ToLower()))
            {
                return false;
            }
