...
namespace KleeneStar.Core.WebUri
{
    ...
    public class ClassIdUriPathSegmentVariable<TParameter> : UriPathSegmentVariableGuid<TParameter>
        where TParameter : IParameterStatic, new()
    {
        ...
        public ClassIdUriPathSegmentVariable(object tag = null)
            : base(tag)
        {
        }
        ...
        {
            return new ClassIdUriPathSegmentVariable<TParameter>(this)
            {
        ...
        public override bool IsMatched(string value)
        {
            ...
            else if (ClassManager.ReservedClassNames.Contains(value?.Trim().ToLower()))
            {
                return false;
            }
