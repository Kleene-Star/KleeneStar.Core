...
namespace KleeneStar.Core.WebUri
{
    ...
    public class WorkflowIdUriPathSegmentVariable<TParameter> : UriPathSegmentVariableGuid<TParameter>
        where TParameter : IParameterStatic, new()
    {
        ...
        public WorkflowIdUriPathSegmentVariable(object tag = null)
            : base(tag)
        {
        }
        ...
        {
            return new WorkflowIdUriPathSegmentVariable<TParameter>(this)
            {
        ...
        public override bool IsMatched(string value)
        {
            ...
            else if (WorkflowManager.ReservedWorkflowNames.Contains(value?.Trim().ToLower()))
            {
                return false;
            }
