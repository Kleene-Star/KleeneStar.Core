...
namespace KleeneStar.Core.WebUri
{
    ...
    public class FieldIdUriPathSegmentVariable<TParameter> : UriPathSegmentVariableGuid<TParameter>
        where TParameter : IParameterStatic, new()
    {
        ...
        public FieldIdUriPathSegmentVariable(object tag = null)
            : base(tag)
        {
        }
        ...
        {
            return new FieldIdUriPathSegmentVariable<TParameter>(this)
            {
        ...
        public override bool IsMatched(string value)
        {
            ...
            else if (FieldManager.ReservedFieldNames.Contains(value?.Trim().ToLower()))
            {
                return false;
            }
